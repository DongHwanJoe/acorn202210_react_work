    ...
    render() {
        ...
        return(
            <div className="container">
                ...
                <button onClick={()=>{
                    this.setState({
                        obj:{...this.state.obj, backgroundColor:"red"}
                    });
                }}>배경색 바꾸기</button>
                ...
                {/* 
                    jsx에서 { 중괄호 내부 }
                    중괄호 내부는 javascript에서 사용하는 데이터가 참조돼야 한다.
                    ...
                    array => [value1, value2, ...]
                    funtion => () => {}
                    - data를 즉석에서 만들면서 참조 또는 이미 만들어진 데이터를 참조
                */}
                ...
                <div className={`box ${ this.state.isYellow ? "bg-yellow" : ""} `} onClick={this.handleClick}>box8</div>
                <p className={styles["text-red"]}>p요소 입니다.</p>
                <p className={styles["text-bold"]}>p요소 입니다.</p>
                <p className={`${styles["text-red"]} ${styles["text-bold"]}`}>p요소 입니다.</p>
            </div>
        );
    }
    handleClick = () => {
